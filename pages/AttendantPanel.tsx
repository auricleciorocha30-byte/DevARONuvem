...
const AttendantPanel: React.FC<Props> = ({ adminUser, onSelectTable, orders, settings, updateStatus, onLogout, isOffline }) => {
  ...
  const newBalcaoEntregaOrders = useMemo(() => {
    return activeOrders.filter(o => (o.type === 'BALCAO' || o.type === 'ENTREGA') && (o.status === 'AGUARDANDO' || o.status === 'AGUARDANDO_PAGAMENTO' || o.status === 'PENDENTE' || o.status === 'PAGO'));
  }, [activeOrders]);
                ...
                <button 
                  ...
                  className={`relative rounded-2xl p-2.5 text-left border flex flex-col justify-between transition-all active:scale-95 shadow-sm overflow-hidden h-[80px] md:h-[90px] ${
                    occ?.status === 'PRONTO' 
                      ...
                      : occ 
                      ? 'bg-secondary border-secondary/80 shadow-secondary/20' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in max-h-[calc(100vh-200px)] overflow-y-auto pr-2 pb-10 custom-scrollbar">
            {displayOrders.map(order => (
              <div key={order.id} className={`bg-white rounded-[2.5rem] p-6 shadow-xl flex flex-col border relative group ${((order.type === 'BALCAO' || order.type === 'ENTREGA') && (order.status === 'AGUARDANDO' || order.status === 'PAGO')) ? 'border-red-400 ring-2 ring-red-100' : 'border-gray-100'}`}>
                {((order.type === 'BALCAO' || order.type === 'ENTREGA') && (order.status === 'AGUARDANDO' || order.status === 'AGUARDANDO_PAGAMENTO' || order.status === 'PENDENTE' || order.status === 'PAGO')) && (
                  ...
                  <div className="flex gap-2">
                    {(order.status === 'AGUARDANDO' || order.status === 'AGUARDANDO_PAGAMENTO' || order.status === 'PENDENTE' || order.status === 'PAGO') && (
                      <div className="flex gap-2 w-full">
                        ...
                        >
                          {isUpdating === order.id ? <Loader2 className="animate-spin" size={14} /> : ((order.status === 'AGUARDANDO_PAGAMENTO' || order.status === 'PENDENTE') ? 'CONFIRMAR E PRODUZIR' : 'MANDAR P/ PRODUÇÃO')}
                        </button>
